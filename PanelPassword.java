...
import javax.swing.*;
import System.PanelPassword.ChangeActionListener;
import System.PanelPassword.MyActionListener;
...
class PanelPassword extends JPanel {
	...
	public PanelPassword(ParkingSystemGui win) {
		setLayout(null);
		this.win = win;
	...
	class OurActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {		
			char[] Test = t.getPassword();
			String change = "";
            
			for (int j = 0; j < Test.length; j++) {
			    change+= Character.toString(Test[j]);
			}
			
			win.change("PanelParking",change);
			t.setText("");
			operator = "";
		}
	}
